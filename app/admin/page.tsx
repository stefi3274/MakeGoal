...
const CATEGORIES = ['Résultat','Double chance','Buts','Trophée','Tirs','Tirs cadrés','Corners','Cartons','Hors-jeu','Buteur','Premier buteur','Score exact'];
type Pari = { niveau: string; categorie: string; type_pari: string; valeur: string; cote: number | null; confiance: number | null; ordre: number; };
...
export default function Admin() {
  ...
  const [pariActuel, setPariActuel] = useState<Pari>({niveau:'Classique',categorie:'Résultat',type_pari:'',valeur:'',cote:null,confiance:null,ordre:0});
  ...
  const analyserAvecIA = async () => {
    if (!match) { setMessage('Entre le nom du match avant d\'analyser.'); return; }
    setAnalysing(true); setMessage('');
    try {
      const res = await fetch('/api/admin/analyse', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({match,competition,date_match:dateMatch})});
      const data = await res.json();
      if (data.success && data.analyse) {
        setContexte(data.analyse.contexte || '');
        setConfiance(data.analyse.confiance_globale || 3);
        if (data.analyse.paris?.length > 0) { setParis(data.analyse.paris.map((p:Pari,i:number) => ({...p,ordre:i+1}))); }
        setMessage('Analyse IA terminée ! Vérifiez et ajustez avant de publier.');
      } else { setMessage('Erreur IA : ' + (data.error || 'inconnue')); }
    } catch { setMessage('Erreur de connexion IA.'); }
    setAnalysing(false);
  };
  const ajouterPari = () => {
  ...
  const sauvegarder = async (publier:boolean) => {
    if (!match || !dateMatch) { setMessage('Match et date obligatoires.'); return; }
    setSaving(true); setMessage('');
    const res = await fetch('/api/admin/pronostic',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({match,competition,date_match:dateMatch,lieu,contexte,confiance_globale:confiance,paris})});const data = await res.json();
    ...
    if (data.success) {
      ...
      setMatch(''); setCompetition(''); setDateMatch(''); setLieu(''); setContexte(''); setConfiance(3); setParis([]);
    ...
    <div style={{minHeight:'100vh',background:'#f9fafb',fontFamily:'sans-serif'}}>
      <header style={{background:'#111',padding:'12px 24px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        ...
        <div style={{display:'flex',gap:'12px',alignItems:'center'}}>
          {vue==='nouveau' && <button onClick={() => setVue('dashboard')} style={{background:'#333',color:'#fff',border:'none',padding:'8px 16px',borderRadius:'8px',cursor:'pointer',fontSize:'13px'}}>← Dashboard</button>}
      ...
      {vue==='dashboard' && (
        <main style={{maxWidth:'800px',margin:'0 auto',padding:'32px 16px'}}>
          ...
          {pronostics.length===0 && <p style={{color:'#6b7280'}}>Aucun pronostic.</p>}{pronostics.map(p => (
      ...
      {vue==='nouveau' && (
        <main style={{maxWidth:'800px',margin:'0 auto',padding:'32px 16px'}}>
          ...
          {message && <p style={{fontWeight:700,marginBottom:'16px',padding:'12px',background:'#f0fdf4',borderRadius:'8px',color:'#10b981'}}>{message}</p>}
          <div style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:'12px',padding:'20px',marginBottom:'16px'}}>
            ...
            {[{label:'Match *',value:match,set:setMatch,placeholder:'PSG vs Arsenal'},{label:'Compétition',value:competition,set:setCompetition,placeholder:'Champions League'},{label:'Lieu',value:lieu,set:setLieu,placeholder:'Budapest, Hongrie'}].map(f => (
            ...
            <button onClick={analyserAvecIA} disabled={analysing || !match} style={{width:'100%',padding:'14px',background:analysing?'#9ca3af':'#111',color:'#fff',border:'none',borderRadius:'12px',cursor:analysing?'not-allowed':'pointer',fontWeight:700,fontSize:'15px',marginBottom:'8px'}}>
              {analysing ? '🤖 Analyse en cours...' : '🤖 Analyser avec l\'IA'}
            </button>
            <p style={{fontSize:'12px',color:'#9ca3af',textAlign:'center',margin:0}}>Entre le match et clique pour générer automatiquement l'analyse et les paris</p>
          </div>
          <div style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:'12px',padding:'20px',marginBottom:'16px'}}>
            <h3 style={{fontWeight:700,marginBottom:'16px',fontSize:'16px'}}>Analyse et confiance</h3>
            <div style={{marginBottom:'12px'}}>
              <label style={{fontSize:'13px',color:'#6b7280',display:'block',marginBottom:'4px'}}>Contexte / Analyse</label>
              <textarea value={contexte} onChange={e => setContexte(e.target.value)} rows={4} placeholder="L'IA remplira ce champ, ou écris ton analyse..." style={{width:'100%',padding:'10px',borderRadius:'8px',border:'1px solid #e5e7eb',fontSize:'14px',boxSizing:'border-box',resize:'vertical'}}/></div>
            <div>
          ...
          <div style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:'12px',padding:'20px',marginBottom:'16px'}}>
            <h3 style={{fontWeight:700,marginBottom:'16px',fontSize:'16px'}}>Ajouter un pari manuellement</h3>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px',marginBottom:'12px'}}>
              <div>
                <label style={{fontSize:'13px',color:'#6b7280',display:'block',marginBottom:'4px'}}>Niveau</label>
                <select value={pariActuel.niveau} onChange={e => setPariActuel({...pariActuel,niveau:e.target.value})} style={{width:'100%',padding:'10px',borderRadius:'8px',border:'1px solid #e5e7eb',fontSize:'14px'}}>
                  {NIVEAUX.map(n => <option key={n}>{n}</option>)}
                </select>
              </div>
              <div>
                <label style={{fontSize:'13px',color:'#6b7280',display:'block',marginBottom:'4px'}}>Catégorie</label>
                <select value={pariActuel.categorie} onChange={e => setPariActuel({...pariActuel,categorie:e.target.value})} style={{width:'100%',padding:'10px',borderRadius:'8px',border:'1px solid #e5e7eb',fontSize:'14px'}}>
            ...
            <div style={{marginBottom:'12px'}}>
              ...
              <input value={pariActuel.valeur} onChange={e => setPariActuel({...pariActuel,valeur:e.target.value})} placeholder="Ex: Victoire PSG, Over 2.5 buts..." style={{width:'100%',padding:'10px',borderRadius:'8px',border:'1px solid #e5e7eb',fontSize:'14px',boxSizing:'border-box'}}/>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px',marginBottom:'16px'}}>
              <div>
                <label style={{fontSize:'13px',color:'#6b7280',display:'block',marginBottom:'4px'}}>Cote</label>
                <input type="number" step="0.01" value={pariActuel.cote||''} onChange={e => setPariActuel({...pariActuel,cote:Number(e.target.value)})} placeholder="2.39" style={{width:'100%',padding:'10px',borderRadius:'8px',border:'1px solid #e5e7eb',fontSize:'14px',boxSizing:'border-box'}}/>
              </div>
              <div>
                <label style={{fontSize:'13px',color:'#6b7280',display:'block',marginBottom:'4px'}}>Confiance (1-5)</label>
                <input type="number" min={1} max={5} value={pariActuel.confiance||''} onChange={e => setPariActuel({...pariActuel,confiance:Number(e.target.value)})} placeholder="4" style={{width:'100%',padding:'10px',borderRadius:'8px',border:'1px solid #e5e7eb',fontSize:'14px',boxSizing:'border-box'}}/>
              </div>
            </div>
            <button onClick={ajouterPari} style={{background:'#111',color:'#fff',border:'none',padding:'10px 24px',borderRadius:'999px',cursor:'pointer',fontWeight:700,fontSize:'14px'}}>+ Ajouter ce pari</button>
          </div>
          {paris.length > 0 && (
            <div style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:'12px',padding:'20px',marginBottom:'16px'}}>
              <h3 style={{fontWeight:700,marginBottom:'12px',fontSize:'16px'}}>{paris.length} paris</h3>
              {paris.map((p,i) => (
                <div key={i} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 0',borderBottom:'1px solid #f3f4f6'}}><div>
                    ...
                    {p.cote && <span style={{color:VIOLET,fontWeight:700,marginLeft:'8px'}}>{Number(p.cote).toFixed(2)}</span>}
                  </div>
