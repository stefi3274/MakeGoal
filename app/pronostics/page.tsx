...
type Pari = {
  id: string;
  niveau: 'Classique' | 'Simple' | 'Complexe' | 'Divin';
  categorie: string;
  type_pari: string;
  valeur: string;
  cote: number | null;
...
const VIOLET = '#bf00ff';
const NIVEAUX = [
  { key: 'Classique', label: 'Classique', color: '#10b981', desc: '🟢 Buts, résultats, double chance.' },
  { key: 'Simple', label: 'Simple', color: '#3b82f6', desc: '🔵 Stats : tirs, cartons, corners, hors-jeu.' },
  { key: 'Complexe', label: 'Complexe', color: '#f59e0b', desc: '🟠 Buteurs et premier buteur.' },
  { key: 'Divin', label: 'Divin ✨', color: '#eab308', desc: '🟡 Maxima et minima — paris premium.' },
] as const;
type Niveau = 'Classique' | 'Simple' | 'Complexe' | 'Divin';
export default function Pronostics() {
  const [pronostics, setPronostics] = useState<Pronostic[]>([]);
  const [loading, setLoading] = useState(true);
  const [niveauActif, setNiveauActif] = useState<Niveau>('Classique');
  useEffect(() => {
  ...
  const descNiveau = (niv: string) => NIVEAUX.find(n => n.key === niv)?.desc || '';
  return (
    <div style={{minHeight:'100vh',background:'#ffffff',color:'#111111',fontFamily:'sans-serif'}}>
      ...
      <main style={{maxWidth:'960px',margin:'0 auto',padding:'48px 24px'}}>
        ...
        <p style={{color:'#6b7280',fontSize:'16px',marginBottom:'32px'}}>Analyses raisonnées en 4 niveaux. Cotes GoalCash Haïti. À titre indicatif.</p>
        ...
        {!loading && pronostics.map((p) => {
          ...
          return (
            <article key={p.id} style={{border:'1px solid #e5e7eb',borderRadius:'16px',padding:'24px',marginBottom:'24px'}}>
              ...
              <p style={{fontSize:'12px',color:'#6b7280',marginBottom:'12px',fontStyle:'italic'}}>{descNiveau(niveauActif)}</p>
        ...
        <div style={{marginTop:'32px',padding:'20px',background:'#fef2f2',borderRadius:'12px',borderLeft:'4px solid #dc2626'}}>
          <p style={{fontSize:'13px',color:'#374151',margin:0,lineHeight:'1.6'}}>
            <strong>Rappel :</strong> Pronostics à titre indicatif. Cotes GoalCash Haïti. Aucune garantie de gain. 18+ uniquement. <a href="/jeu-responsable" style={{color:VIOLET}}>Jeu responsable</a>.
          </p>
        </div>
      </main>
      <Footer />
